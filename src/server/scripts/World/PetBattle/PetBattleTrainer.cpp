            ...
            if (isTrainer)
            {
                ...
                if (check)
                    if (BroadcastTextEntry const* bct = sBroadcastTextStore.LookupEntry(62660))
                        player->ADD_GOSSIP_ITEM(GOSSIP_NPC_CHAT, DB2Manager::GetBroadcastTextValue(bct, player->GetSession()->GetSessionDbLocaleIndex()), GOSSIP_SENDER_MAIN, PetBattleTrainerFightActionID);
            }
