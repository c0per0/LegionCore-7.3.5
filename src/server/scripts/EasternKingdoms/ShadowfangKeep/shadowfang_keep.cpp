        ...
        bool OnGossipHello(Player* player, Creature* pCreature)
        {
            ...
            if (InstanceScript* instance = pCreature->GetInstanceScript())
            {
                if (instance->GetBossState(DATA_VALDEN) == DONE)
                    player->ADD_GOSSIP_ITEM(GOSSIP_NPC_CHAT, ru ? "Teleport to Lower Observatory." : "Teleport to Lower Observatory.", GOSSIP_SENDER_SHADOWFANG_PORT, 3);
                else if (instance->GetBossState(DATA_SPRINGVALE) == DONE)
                    player->ADD_GOSSIP_ITEM(GOSSIP_NPC_CHAT, ru ? "Teleport to The Courtyard." : "Teleport to The Courtyard.", GOSSIP_SENDER_SHADOWFANG_PORT, 2);
                else if (instance->GetBossState(DATA_SILVERLAINE) == DONE)
                    player->ADD_GOSSIP_ITEM(GOSSIP_NPC_CHAT, ru ? "Teleport to Dining Hall." : "Teleport to Dining Hall.", GOSSIP_SENDER_SHADOWFANG_PORT, 1);
                else if (instance->GetBossState(DATA_ASHBURY) == DONE)
                    player->ADD_GOSSIP_ITEM(GOSSIP_NPC_CHAT, ru ? "Teleport to The Courtyard." : "Teleport to The Courtyard.", GOSSIP_SENDER_SHADOWFANG_PORT, 0);
            }
