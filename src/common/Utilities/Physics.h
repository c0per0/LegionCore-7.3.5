...
#ifndef TRINITY_PHYSICS_H
#define TRINITY_PHYSICS_H
...
inline float getWeight(float height, float width, float specificWeight)
{
 ...
 * @return float
 */
inline float getOutOfWater(float width, float weight, float density)
{
    auto baseArea = getCircleAreaByRadius(width / 2.0f);
    return weight / (baseArea * density);
}

#endif // TRINITY_PHYSICS_H